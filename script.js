...
contactForm.addEventListener('submit', async function(e) {
    ...
    try {
        // Send to backend (simple relative URL since we're serving from the same server)
        ...
        if (result.success) {
            showNotification('> MESSAGE TRANSMITTED SUCCESSFULLY! <br>> I will respond within 24 hours', 'success');
            this.reset();
            
            // Add cyber effect to form
        ...
        const mailtoLink = `mailto:mystechdevelopment@proton.me?subject=Portfolio Contact from ${encodeURIComponent(data.name)}&body=Name: ${encodeURIComponent(data.name)}%0AEmail: ${encodeURIComponent(data.email)}%0A%0AMessage:%0A${encodeURIComponent(data.message)}`;
        
        showNotification(
            `Backend temporarily offline. <a href="${mailtoLink}" style="color: #00ffff; text-decoration: underline;">Click here to send via email client</a>`,
            'error'
        );
    } finally {
        // Reset button state
        ...
        contactForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    formStatus.textContent = errorResult.error || 'An error occurred. Please try again.';
                }
            } catch (error) {
                console.error('Error submitting form:', error);
                formStatus.textContent = 'An error occurred. Please check the console for details.';
            }
        });
    }
});
